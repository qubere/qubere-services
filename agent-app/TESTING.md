...
By default this uses the in-memory queue. To test the durable database queue instead, start the app with `AGENT_PLATFORM_ASYNC_QUEUE_TYPE=database` after adding the `agent_async_queue_command` table from the manual DDL.

...
public class InMemoryAgentApprovalStore implements AgentApprovalStore {
    ...
    public Collection<AgentApprovalRequest> list(AgentApprovalStatus status, String tenantId, int limit) {
        ...
        return approvals.values().stream()
                .filter(approval -> status == null || approval.status() == status)
                .filter(approval -> tenantId == null || tenantId.isBlank() || tenantId.equals(approval.tenantId()))
                .sorted(Comparator.comparing(AgentApprovalRequest::createdAt).reversed())
                .limit(safeLimit)
                .toList();
    }
    ...
    public int expirePendingBefore(Instant now) {
        ...
        return (int) approvals.values().stream()
                .filter(approval -> approval.status() == AgentApprovalStatus.PENDING)
                .filter(approval -> approval.expiresAt() != null && !approval.expiresAt().isAfter(cutoff))
                .map(approval -> expire(approval.approvalId()))
                .count();
    }

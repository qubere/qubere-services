...
public class JpaAgentApprovalStore implements AgentApprovalStore {
    ...
    @Override
    @Transactional(readOnly = true)
    public Collection<AgentApprovalRequest> list(AgentApprovalStatus status, String tenantId, int limit) {
        PageRequest page = PageRequest.of(0, Math.max(1, limit));
        if (tenantId != null && !tenantId.isBlank() && status != null) {
            return repository.findByTenantIdAndStatusOrderByCreatedAtDesc(tenantId, status, page).stream().map(this::toRequest).toList();
        }
        if (tenantId != null && !tenantId.isBlank()) {
            return repository.findByTenantIdOrderByCreatedAtDesc(tenantId, page).stream().map(this::toRequest).toList();
        }
        if (status != null) {
            return repository.findByStatusOrderByCreatedAtDesc(status, page).stream().map(this::toRequest).toList();
        }
        return repository.findAllByOrderByCreatedAtDesc(page).stream().map(this::toRequest).toList();
    }

    @Override
    @Transactional
    public int expirePendingBefore(Instant now) {
        Instant cutoff = now == null ? Instant.now() : now;
        return repository.findByStatusAndExpiresAtLessThanEqual(AgentApprovalStatus.PENDING, cutoff).stream()
                .map(entity -> {
                    entity.setStatus(AgentApprovalStatus.EXPIRED);
                    entity.setDecidedBy("system");

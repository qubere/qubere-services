...
The current observability service keeps recent pipeline events in memory for admin summaries, and `agent-storage` now persists execution timelines, tool calls, and model usage through `agent_execution_log`, `agent_tool_call`, and `agent_model_usage`.
...
- Add an OpenTelemetry bridge for traces/metrics/logs.
...
Async execution and human approval work in the reference app. The default queue is in-memory; `agent-platform.async.queue.type=database` enables a JPA-backed queue using `agent_async_queue_command`. Pending commands and approvals can also be persisted through JPA. The reference app now includes operational admin endpoints for approval review and expiry processing.
...
- Add external production queue adapters such as Kafka, RabbitMQ, or SQS.
...
- Provide reference tenant/actor authorization implementation.
...
9. Add OpenTelemetry and production observability adapters.
